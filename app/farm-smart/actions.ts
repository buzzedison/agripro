...
export async function submitFarmSmartForm(data: FarmSmartFormData) {
  try {
    ...
    const adminEmailResult = await resend.emails.send({
      ...
      to: ADMIN_EMAILS,
      subject: '🌱 New Farm Smart Program Application',
      html: `
        <h1>New Farm Smart Program Application</h1>
        ...
        <ul>
          ...
          <li><strong>Interested in Micro Grant:</strong> ${data.interestedInGrant ? 'Yes' : 'No'}</li>
          ${data.interestedInGrant ? `<li><strong>Grant Amount Requested:</strong> ₵${data.grantAmount}</li>` : ''}
        </ul>
        ...
        html: `
          <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e0e0e0; border-radius: 10px;">
            ...
            <div style="background-color: #f1f8e9; padding: 15px; border-radius: 8px; margin: 20px 0;">
              <p style="margin: 0; font-weight: bold;">Program Dates:</p>
              <p style="margin: 10px 0 0;">Saturdays, 17 & 24 May 2025 at Enterprise Village, Accra</p>
            </div>
            
            ${data.interestedInGrant ? `
            <div style="background-color: #e8f5e9; padding: 15px; border-radius: 8px; margin: 20px 0;">
              <p style="margin: 0; font-weight: bold;">Micro Grant Application:</p>
              <p style="margin: 10px 0 0;">We've noted your interest in applying for a micro grant of ₵${data.grantAmount}. Our team will provide more information about the grant application process in our follow-up communication.</p>
            </div>
            ` : ''}
