    ...
    return (
        <>
            ...
            <button
                ...
                className="fixed bottom-6 right-6 z-40 flex items-center gap-2 px-4 py-3 bg-gray-900 text-white rounded-full shadow-2xl hover:bg-gray-800 transition-all text-sm font-bold"
            >
                ...
                {open && (
                    <>
                        ...
                        <motion.div
                            ...
                            className="fixed bottom-6 right-6 z-50 w-[calc(100vw-3rem)] sm:w-96 max-h-[75vh] bg-gray-950 border border-white/10 rounded-3xl shadow-2xl flex flex-col overflow-hidden"
                        >
                            {/* Header */}
                            <div className="flex items-center justify-between px-4 py-3.5 border-b border-white/5 shrink-0">
                                <div className="flex items-center gap-2">
                                    ...
                                    <span className="text-white font-bold text-sm">Ask about this article</span>
                                </div>
                                        ...
                                        <div className="space-y-2">
                                            {STARTERS.map((s) => (
                                                <button
                                                    key={s}
                                                    onClick={() => send(s)}
                                                    className="block w-full text-left px-3 py-2 bg-white/5 border border-white/8 rounded-xl text-xs text-gray-400 hover:text-white hover:bg-white/10 transition-all"
                                                >
                                                    {s}
                                                </button>
                                            ))}
                            ...
                            <div className="px-3 py-3 border-t border-white/5 shrink-0">
                                <div className="flex items-center gap-2 bg-white/5 border border-white/10 rounded-xl px-3 py-2 focus-within:border-green-500/40 transition-all">
                                    <input
                                        value={input}
                                    ...
                                    <button
                                        ...
                                        className="w-6 h-6 rounded-lg bg-white disabled:bg-white/10 flex items-center justify-center transition-all"
                                    >
                                        <Send className={`w-3 h-3 ${input.trim() && !loading ? 'text-gray-900' : 'text-gray-600'}`} />
                                    </button>
                                </div>
                            </div>
                        </motion.div>
