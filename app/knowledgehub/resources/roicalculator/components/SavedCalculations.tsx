...
// Define the calculation result types from Supabase
interface SupabaseCalculation {
  id: string;
  title: string;
...
interface SavedCalculationsProps {
  activeTab: string;
  // Using a string-based approach for client components
  onTabChange?: string; // Event name for tab change
}

export default function SavedCalculations({ activeTab, onTabChange }: SavedCalculationsProps) {
  // Client-side function to handle tab changes
  const handleTabChange = (tab: string) => {
    if (onTabChange) {
      // Use client-side event handling
      const event = new CustomEvent(onTabChange, { detail: { tab } });
  ...
  useEffect(() => {
    ...
    if (savedCalculationsJSON) {
      try {
        ...
        toast.error('Error loading saved calculations', {
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <div className="flex flex-wrap gap-2">
              <Tabs defaultValue="all" className="w-[200px]" onValueChange={(v) => setFilterType(v as any)}>
                <TabsList className="grid grid-cols-3" role="tablist">
                  <TabsTrigger value="all" role="tab" aria-selected={filterType === 'all'}>All</TabsTrigger>
                  <TabsTrigger value="crop" role="tab" aria-selected={filterType === 'crop'}>Crops</TabsTrigger>
                  <TabsTrigger value="livestock" role="tab" aria-selected={filterType === 'livestock'}>Livestock</TabsTrigger>
                </TabsList>
                {/* Adding TabsContent elements to satisfy ARIA requirements */}
                <div className="sr-only">
                  <TabsContent value="all" role="tabpanel">All calculations</TabsContent>
                  <TabsContent value="crop" role="tabpanel">Crop calculations</TabsContent>
                  <TabsContent value="livestock" role="tabpanel">Livestock calculations</TabsContent>
                </div>
              </Tabs>
              ...
              <TableBody>
                {filteredCalculations.map((calculation, index) => (
                  <TableRow key={index} className={selectedCalculations.includes(calculation.name) ? 'bg-muted/50' : ''}>
                    <TableCell>
                        <div className="flex items-center">
                          <input 
                            ...
                            className="h-4 w-4"
                            aria-label={`Select ${calculation.name}`}
                          />
                          <label htmlFor={`select-calculation-${index}`} className="sr-only">
                            Select {calculation.name}
                          </label>
                        </div>
                    </TableCell>
                    ...
                    <TableCell className={`text-right ${(calculation.roi || 0) >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {(calculation.roi || 0).toFixed(2)}%
                    </TableCell>
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent>
                          <AlertDialogHeader>
                            <AlertDialogTitle>Are you sure?</AlertDialogTitle>
                            <AlertDialogDescription>
                              This action will delete the calculation &ldquo;{calculation.name}&rdquo;. This action cannot be undone.
                            </AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>Cancel</AlertDialogCancel>
                  ...
                  <Table>
                    ...
                    <TableBody>
                      ...
                      <TableRow>
                        ...
                        {selectedCalculationsData.map((calc, i) => (
                          <TableCell key={i}>{formatCurrency(calc.totalCost || 0)}</TableCell>
                        ))}
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">Total Revenue</TableCell>
                        {selectedCalculationsData.map((calc, i) => (
                          <TableCell key={i}>{formatCurrency(calc.totalRevenue || 0)}</TableCell>
                        ))}
                      </TableRow>
                      <TableRow>
                        <TableCell className="font-medium">ROI</TableCell>
                        {selectedCalculationsData.map((calc, i) => (
                          <TableCell key={i} className={(calc.roi || 0) >= 0 ? 'text-green-600' : 'text-red-600'}>
                            {(calc.roi || 0).toFixed(2)}%
                          </TableCell>
                        ))}
                      </TableRow>
                      ...
                      <TableRow>
                        ...
                        {selectedCalculationsData.map((calc, i) => (
                          <TableCell key={i}>
                            {calc.type === 'crop' ? (
                              <>
                                <span className="block">Crop: {calc.cropType || 'Unknown'}</span>
                                <span className="block">Area: {calc.inputs?.area || 0} ha</span>
                                <span className="block">Yield: {calc.inputs?.expectedYield || 0} tons/ha</span>
                              </>
                            ) : (
                              <>
                                <span className="block">Livestock: {calc.livestockType || 'Unknown'}</span>
                                <span className="block">Animals: {calc.inputs?.numAnimals || 0}</span>
                                <span className="block">Type: {calc.productionType || 'Unknown'}</span>
                              </>
                            )}
                          </TableCell>
                        ))}
                        ...
                        <div className="space-y-4">
                          ...
                          <div className="space-y-2">
                            <div className="flex justify-between">
                              ...
                              <span>{formatCurrency(calc.totalCost || 0)}</span>
                            </div>
                            <div className="flex justify-between">
                              <span className="text-muted-foreground">Total Revenue:</span>
                              <span>{formatCurrency(calc.totalRevenue || 0)}</span>
                            </div>
                            <div className="flex justify-between font-semibold">
                              <span>ROI:</span>
                              <span className={(calc.roi || 0) >= 0 ? 'text-green-600' : 'text-red-600'}>
                                {(calc.roi || 0).toFixed(2)}%
                              </span>
                            </div>
                          </div>
                            ...
                            {calc.type === 'crop' ? (
                              <div className="space-y-1 text-sm">
                                <div className="flex justify-between">
                                  ...
                                  <span>{calc.cropType || 'Unknown'}</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-muted-foreground">Area:</span>
                                  <span>{calc.inputs?.area || 0} ha</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-muted-foreground">Yield:</span>
                                  <span>{calc.inputs?.expectedYield || 0} tons/ha</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-muted-foreground">Price:</span>
                                  <span>{formatCurrency(calc.inputs?.sellingPrice || 0)}/ton</span>
                                </div>
                              </div>
                            ) : (
                              <div className="space-y-1 text-sm">
                                <div className="flex justify-between">
                                  ...
                                  <span>{calc.livestockType || 'Unknown'}</span>
                                </div>
                                <div className="flex justify-between">
                                  <span className="text-muted-foreground">Animals:</span>
                                  <span>{calc.inputs?.numAnimals || 0}</span>
                                </div>
                                <div className="flex justify-between">
                                ...
                                <div className="flex justify-between">
                                  ...
                                  <span>
                                    {calc.inputs?.productionAmount || 0} 
